...
(() => {
  ...
  const setMarketPool = (value) => {
    ...
    update("marketPool", next);
    core.storage.write(poolKey, { enabledIds: next.desiredIds, runningIds: next.currentIds, desiredIds: next.desiredIds, currentIds: next.currentIds, nextRoundIds: next.nextRoundIds });
    return next;
  };
  const setMarketCatalog = (value) => {
    ...
    update("marketCatalog", { ...next, status: next.error ? "error" : "ready", selectedId: selected, receivedAt: Date.now() });
    update("marketPool", vm.pool(state.marketPool, next.items));
    return next;
  };
