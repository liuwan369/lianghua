...
前端 `npm --prefix web test` 为 71 项通过，构建通过。功能架构覆盖 schema 校验、重复 ID、失败清空、刷新保留筛选及选择、旧任务数据兼容、平台运行身份、过期快照和 PAUSED 状态。本轮桌面/手机检查无横向溢出和浏览器错误。离线模拟接口用于本地浏览器检查，不构成真实交易测试。
