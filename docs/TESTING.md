...
覆盖真实 `run()` 主循环收到成交后在无下一盘口或心跳时主动补单、成交处理到强制补单的组件链路不调用 REST、旧报价占用清理、慢账户 TTL 后仍可创建资金预留，以及后续订单状态合并写入且不逐次 fsync。主循环用例保留实际 Engine/Executor，账户启动和网络边界用替身；通过内存变换移除即时重评估后，该用例会因只提交首单而失败。测试使用临时目录，不连接真实账户，不产生交易；它们检查依赖和执行顺序，不把本机用例耗时当成交易所延迟。
...
账户链路新增：跨账户/过期清空、分页与只读协议、Maker归属与未知方向、CSV公式转义、纸面控制不解锁实盘、重复点击/不确定请求复用与版本冲突恢复。EXEC-02 离线权益契约定向 53 项通过（权益 23、风险存储 21、财务 9），含存量持仓、充值提款、北京时间结转、粘滞 30 美元停机及严格恢复检查；尚未接权威快照适配、原子预留或下单门禁。服务器独立只读查询已成功；Windows Computer Use 的网址安全检查停止了操作，本轮不得标记真实浏览器验收通过。
