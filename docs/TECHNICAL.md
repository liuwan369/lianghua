...
更新时间：2026-09-17。本文以 `_external/btc-5m-market-trading-bot/src/platform/` 当前源码为准；旧 `src/live/engine.ts` 的策略参数不属于公共底座默认值。主线已转为 [BTC 五分钟反转开发规划](REVERSAL-DELIVERY-PLAN-2026-09-17.md)，下文保留当前实现语义，不能将规划中的插件、自动换场和真实启停视为已实现。
...
现有六页配置仍可保存九项后台字段，这些旧字段不构成反转策略参数。新策略通过 `StrategyPlugin` 接入，不能直接导入 CLOB client、写账本或修改风险上限。旧做市停止开发；已授权的 BTC 五分钟反转插件、70/75 互斥配置和阶段持久化将与底座恢复补齐并行实现，不再等待整个平台审查结束。当前插件尚不存在，控制台仍只启动无策略 paper 观察。
