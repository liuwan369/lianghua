...
官方来源核对日期：2026-09-09。六页收益模块的规则展示和独立试算使用这些来源；真实账户奖励账单与付款凭证尚未接入。规则核对不等于账户资格或到账确认，活动是否继续有效应按新的官方证据更新。
...
核对日官方明确有 Taker Rebate Program，自 2026-05-28 生效，并列有首次升级奖金。页面按有来源的规则展示；未知项目标记未核实，不填写传闻比例。
...
- 读取实际账户有效档位、30 天 taker wV、奖金领取记录及推荐/Builder 审批；已核对的 API 文档索引未确认所有这些数据的公开接口，不编造路径。
...
- 周期选择在未接账单时只显示未获取状态，不伪造请求或样本金额；接入真实数据后需核验筛选范围与账本一致。

## 试算边界

wV 输入保留原始精度；到下一档的正差额小于 0.01 时显示 `<0.01`，不能显示“还差 0”而仍未升级。临界档位、负数、空值、示例和清空属于前端逻辑校验；它们不证明实际档位升级、领取或付款完成。正式接入验收见 [前端清单](FRONTEND-INTEGRATION-PLAN.md)。
