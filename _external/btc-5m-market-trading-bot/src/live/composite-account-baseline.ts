...
function validateRead(r: CompositeAccountRead, wallet: string): void {
  if (!r || r.wallet.toLowerCase() !== wallet.toLowerCase() || typeof r.snapshot_token !== 'string' || r.snapshot_token.length < 8) throw new Error('account_mismatch');
  isoMs(r.checked_at);
  for (const name of ['collateral', 'positions', 'cashFlows', 'transfers'] as const) {
    const s = r[name]; if (!s || s.available !== true || s.complete !== true || !Array.isArray(s.items) || !s.source || s.snapshot_token !== r.snapshot_token) throw new Error(`incomplete_${name}`);
    isoMs(s.checked_at);
  }
}
...
export async function buildCompositeAccountBaseline(read: () => Promise<CompositeAccountRead>, wallet: string, nowMs = Date.now(), openingWindowMs = 5 * 60_000): Promise<BaselineCut> {
  ...
  if (a.snapshot_token !== b.snapshot_token) throw new Error('account_read_token_changed');
  ...
  if (skew > openingWindowMs) throw new Error('account_read_skew_exceeded');
