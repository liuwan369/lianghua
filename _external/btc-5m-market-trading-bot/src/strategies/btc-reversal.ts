...
export class BtcReversalStrategy implements StrategyPlugin {
  ...
  setPaused(paused: boolean): void {
    ...
    for (const round of this.state.rounds) if (round.status === "running") this.invalidateReference(round);
    this.save();
  }
  ...
  onEvent(event: TradingEvent, context: StrategyContext): readonly StrategyAction[] {
    ...
    for (const order of context.account.orders) changed = this.recordOrder(order) || changed;
    if (event.kind === "error" && event.clientOrderId) {
      const stage = this.state.rounds.flatMap(round => round.stages).find(s => s.clientOrderId === event.clientOrderId);
      ...
      if (stage?.status === "CREATED" && event.code === "order_not_submitted") {
        stage.status = "REJECTED"; this.replayIntents.delete(stage.clientOrderId); changed = true;
      }
    }
    if (event.kind === "error" && event.message === "market_feed_disconnected") this.resetQuoteReference(event.marketId);
    ...
    for (const round of this.state.rounds) {
      ...
      if (this.state.paused || context.account.risk.halted) {
        round.reason = this.state.paused ? "已暂停新增交易" : context.account.risk.reason ?? "账户暂不可交易";
      ...
      const unreservedStages = this.state.rounds.flatMap(r => r.stages).filter(stage => stage.status === "CREATED"
        && !context.account.orders.some(order => order.clientOrderId === stage.clientOrderId));
  ...
  private discover(context: StrategyContext): boolean {
    ...
    for (const market of context.markets) {
      if (this.state.rounds.some(round => round.marketId === market.id) || context.now >= market.endsAt) continue;
  ...
  private fresh(book: Book, now: number, round: ReversalRound): boolean {
    const sourceTs = book.exchangeTs ?? book.ts, received = book.receivedAt ?? book.ts;
    return positive(book.ask) && book.ask < 1 && Number.isFinite(sourceTs) && Number.isFinite(received)
      ...
      && sourceTs <= now + round.config.maxQuoteSkewSeconds && received <= now + round.config.maxQuoteSkewSeconds
      && now - sourceTs <= round.config.maxQuoteAgeSeconds && now - received <= round.config.maxQuoteAgeSeconds;
  }
  ...
  private recordOrder(order: OrderRecord): boolean {
    ...
    const stage = this.state.rounds.flatMap(round => round.stages).find(s => s.clientOrderId === order.clientOrderId);
    ...
    for (const round of state.rounds) {
      ...
      for (const [index, stage] of round.stages.entries()) {
        ...
        if (stage.stage !== index + 1 || stage.clientOrderId !== expectedId || clients.has(stage.clientOrderId)
          ...
          || !["CREATED", "SUBMITTING", "OPEN", "PARTIAL", "FILLED", "CANCELLED", "REJECTED", "UNKNOWN"].includes(stage.status)
