...
function mountStrategies() {
  ...
  const render = (payload: { receivedAt: number; sources: Record<string, unknown>[] }) => {
    ...
    for (const name of ['H1', 'H2']) {
      ...
      const rate = sourceValue(sources, ['candidates', name, 'historical_accuracy']) ?? sourceValue(sources, ['candidates', name, 'live_accuracy']) ?? sourceValue(sources, ['accuracy', name, 'rate']) ?? sourceValue(sources, ['accuracy', `${name.toLowerCase()}_rate`]) ?? sourceValue(sources, ['evaluation', name, 'rate']);
      const hits = sourceValue(sources, ['candidates', name, 'historical_hits']) ?? sourceValue(sources, ['candidates', name, 'live_hits']) ?? sourceValue(sources, ['accuracy', name, 'hits']) ?? sourceValue(sources, ['evaluation', name, 'hits']); const samples = sourceValue(sources, ['candidates', name, 'historical_signals']) ?? sourceValue(sources, ['candidates', name, 'live_signals']) ?? sourceValue(sources, ['accuracy', name, 'samples']) ?? sourceValue(sources, ['evaluation', name, 'samples']);
      text(`[data-strategy-direction="${name}"]`, directionValue); text(`[data-strategy-reason="${name}"]`, directionValue === 'SKIP' ? (sourceValue(sources, ['decisions', name, 'reason']) ?? '条件不足') : '方向已冻结'); text(`[data-strategy-rate="${name}"]`, typeof rate === 'number' ? `${(rate <= 1 ? rate * 100 : rate).toFixed(2)}%` : '--'); text(`[data-strategy-record="${name}"]`, hits != null && samples != null ? `${hits} / ${samples}` : '--'); text(`[data-strategy-last="${name}"]`, directionValue === 'SKIP' ? '无有效信号' : directionValue);
    }
  };
