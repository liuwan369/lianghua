...
it('renders real balances and positions, escapes upstream text, and clears after a read failure',async()=>{
  ...
  const ui=connectAccountData();const orderSource=document.querySelector<HTMLSelectElement>('#orders-source')!;orderSource.value='account';orderSource.dispatchEvent(new Event('change'));ui.receiveAccount({wallet,wallet_configured:true,owner_signer_configured:true,relayer_api_configured:false,builder_api_configured:false,last_check:null,config_error:null,control_source:{scope:'local_preview',label:'本机预览服务',market_node:'都柏林节点'}});
...
it('activates the original filter toolbar and filters real orders versus fills',async()=>{
  ...
  const ui=connectAccountData();const orderSource=document.querySelector<HTMLSelectElement>('#orders-source')!;orderSource.value='account';orderSource.dispatchEvent(new Event('change'));ui.receiveAccount({wallet,wallet_configured:true,owner_signer_configured:true,relayer_api_configured:false,builder_api_configured:false,last_check:null,config_error:null});await ui.refresh();
...
async function mounted(d:AccountData){
  document.body.innerHTML='<div id="app"></div>';mountLayout(document.getElementById('app')!);
  vi.stubGlobal('fetch',vi.fn().mockResolvedValue(new Response(JSON.stringify(d))));
  const ui=connectAccountData();const orderSource=document.querySelector<HTMLSelectElement>('#orders-source')!;orderSource.value='account';orderSource.dispatchEvent(new Event('change'));ui.receiveAccount({wallet,wallet_configured:true,owner_signer_configured:true,relayer_api_configured:false,builder_api_configured:false,last_check:null,config_error:null});await ui.refresh();return ui;
}
