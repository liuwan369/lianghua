...
export function connectTradingControls(refresh:()=>Promise<void>) {
  ...
  function render() {
    starts.forEach(b=>{b.disabled=busy||!status||status.running||!strategy||strategy.savedRevision<1;b.textContent=pending?'核对上次启动':'启动 BTC 反转';b.title='使用已保存的参数启动真实交易';});
    stops.forEach(b=>{b.disabled=busy||!status?.running;b.textContent='停止并撤余量';b.title='停止新增并撤销剩余挂单，已成交持仓保留';});
    pauses.forEach(b=>{b.disabled=busy||!status?.running;b.textContent=runtime()?.paused?'恢复新增':'暂停新增';});
  }
  async function control(action:string) {
    ...
    busy=true;render();notice.textContent=action==='start'?'正在启动已保存的 BTC 反转策略…':action==='stop'?'正在停止新增并撤销剩余挂单…':'正在更新运行状态…';
