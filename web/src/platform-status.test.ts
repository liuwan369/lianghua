...
function fixture(now=Date.now()/1000): Status {
  return {schemaVersion:1,asOf:now,running:true,mode:'live',run_id:'platform-one',account_id:'wallet-one',config_revision:1,params:{max_total_usd:10},live_unlocked:true,stop_result:{},execution_target:'platform',engine:'platform',execution:'observation',strategy_id:null,
    control_source:{scope:'collector_host',label:'都柏林服务器',market_node:'Dublin'},
    projection:{run_id:'platform-one',state:'ready',stale:false},stats:{available:true,fills:0,fill_notional:0,events:[],runtime:{engine:'platform',execution:'observation',strategy_id:null,status:'running',mode:'live',source_at:now,expires_at:now+10,stale:false,cash_usd:106.31,positions_count:0,orders_count:0,active_orders:0,fills_count:0,risk:{halted:false},limits:{capitalUsd:1000},markets:[],books:[]}}};
}
it('accepts platform snapshots while preserving legacy status compatibility',()=>{
  ...
  expect(executionName({...status,engine:'legacy',mode:'paper'})).toBe('旧引擎纸面');
});
...
it('renders live platform values separately and removes current values when the snapshot ages',async()=>{
  vi.useFakeTimers();
  ...
  const fixtures:Record<string,unknown>={
    ...
    '/api/v1/config':{schemaVersion:1,revision:1,savedAt:null,params:{mode:'live',duration_min:0},capabilities:{executionTarget:'platform',executionMode:'strategy',runtimeAppliedFields:['mode','duration_min'],preservedLegacyFields:[]}},
  ...
  expect(document.querySelector('[data-runtime-cash]')!.textContent).toBe('$106.31 · 实盘账户');
  ...
  expect(document.querySelector('[data-runtime-cash]')!.textContent).not.toContain('$106.31');
