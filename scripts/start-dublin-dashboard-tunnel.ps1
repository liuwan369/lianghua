...
if ($listener) {
    try {
        Invoke-WebRequest -UseBasicParsing "http://127.0.0.1:$port/console/" -TimeoutSec 3 | Out-Null
        Write-Output "Dublin dashboard already available at http://127.0.0.1:$port/console/"
        exit 0
    } catch {
...
for ($attempt = 0; $attempt -lt 20; $attempt++) {
    try {
        ...
        Write-Output "Dublin dashboard available at http://127.0.0.1:$port/console/"
        exit 0
    } catch {
