...
export function connectForms(saved?: { account: () => void }) {
  ...
  const accountFields = Array.from(document.querySelectorAll<HTMLInputElement>('#settings-account input')).filter(field => field.id !== 'controlToken');
  const accountButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('#settings-account [data-account-action]'));
  const controlToken = document.querySelector<HTMLInputElement>('#controlToken');
  const controlTokenApply = document.querySelector<HTMLButtonElement>('[data-control-token-apply]');
  const controlTokenMessage = document.querySelector<HTMLElement>('[data-control-token-message]');
  ...
  let controlAuthBusy = false;
  let controlTokenApplied = false;
  ...
  function buttons() {
    ...
    if (controlTokenApply) {
      controlTokenApply.disabled = !controlToken?.value.trim() || controlTokenApplied || accountBusy || controlAuthBusy || closed;
      controlTokenApply.textContent = controlTokenApplied ? '控制会话已保存' : '保存控制密码';
    }
  ...
  function receiveAccount(account: Account | null) {
    ...
    // A read started before a successful save may finish afterwards. Do not
    // let that stale response replace the wallet just confirmed by the save.
    if (savedWallet && account.wallet.toLowerCase() !== savedWallet.toLowerCase()) return;
  ...
  if (controlToken && controlTokenApply) {
    on(controlToken, 'input', () => {
      controlTokenApplied = false;
      delete controlToken.dataset.applied;
      if (controlTokenMessage) controlTokenMessage.textContent = controlToken.value.trim() ? '已填写，点击“保存控制密码”后写入服务器私有配置。' : '尚未保存交易控制密码。';
      buttons();
    });
    on(controlTokenApply, 'click', async () => {
      const token = controlToken.value.trim();
      if (!token || controlAuthBusy || closed) return;
      controlAuthBusy = true; buttons();
      if (controlTokenMessage) controlTokenMessage.textContent = '正在保存交易控制密码…';
      try {
        const response = await api.saveControlSession(token);
        if (closed) return;
        controlTokenApplied = true;
        controlToken.dataset.applied = 'true';
        controlToken.value = '';
        if (controlTokenMessage) {
          const seconds = typeof response.expires_in === 'number' ? response.expires_in : 12 * 60 * 60;
          const hours = Math.max(1, Math.round(seconds / 3600));
          controlTokenMessage.textContent = `交易控制密码已保存到服务器私有配置；当前浏览器会话约 ${hours} 小时有效，服务重启后仍可用已保存密码。`;
        }
      } catch (error) {
        if (!closed && controlTokenMessage) controlTokenMessage.textContent = `${error instanceof Error ? error.message : '交易控制密码保存失败'}；密码未保存，请核对后重试。`;
      } finally { controlAuthBusy = false; if (!closed) buttons(); }
    });
  }
  ...
  document.querySelectorAll('[data-setting]').forEach(button => on(button, 'click', () => queueMicrotask(buttons)));
  accountMessage('可通过当前 HTTPS 页面检查、保存账户。空白密钥在钱包不变时保留服务器配置，换钱包不会继承旧密钥。检查不会保存，保存不会启动交易。');
  buttons();
  return { receiveAccount, close: () => { closed = true; bindings.forEach(remove => remove()); accountFields.filter(field => field.type === 'password').forEach(field => field.value = ''); if (controlToken) { controlToken.value = ''; delete controlToken.dataset.applied; } } };
}
