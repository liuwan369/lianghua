...
export function connectStrategy(saved?: (value: StrategyConfig) => void) {
  ...
  function budgetHint() {
    const priceField = root.querySelector<HTMLInputElement>('[data-reversal-field="maxBuyPrice"]');
    const target = root.querySelector<HTMLElement>('[data-reversal-budget-hint]')!;
    const price = Number(priceField?.value) / 100;
    const stageCount = Number(count.value);
    const stages = shares().slice(0, stageCount).map(field => Number(field.value));
    if (!priceField?.value.trim() || !Number.isFinite(price) || price <= 0 || !count.checkValidity() || stages.length !== stageCount || stages.length === 0 || stages.some(value => !Number.isFinite(value) || value <= 0)) {
      target.textContent = '填写最高买入价和阶段份额后显示名义成本；交易费用另计。';
      return;
    }
    const costs = stages.map(value => value * price);
    const total = costs.reduce((sum, value) => sum + value, 0);
    const detail = costs.map((value, index) => `第 ${index + 1} 阶段 $${value.toFixed(2)}`).join('，');
    const boundaries = [['roundBudgetUsd','单场上限'],['totalBudgetUsd','策略总资金上限']].map(([key,label]) => {
      const text = root.querySelector<HTMLInputElement>(`[data-reversal-field="${key}"]`)!.value.trim();
      const value = Number(text);
      if (!text) return `${label}未设置。`;
      if (!Number.isFinite(value) || value <= 0) return `${label}需填写大于 0 的金额或留空。`;
      return `${label} $${value.toFixed(2)}${value < total ? ' 低于全部阶段名义成本，后续买入可能受此额度限制' : ''}。`;
    }).join('');
    target.textContent = `${detail}；按最高买入价计算的全部阶段名义成本 $${total.toFixed(2)}。${boundaries}交易费用另计，实际可用资金还取决于账户余额和已有占用。`;
  }
  ...
  on(form, 'input', () => { markDirty(); budgetHint(); });
  on(count, 'change', () => { if (!count.checkValidity()) return; const previous = shares().map(f => f.value.trim() ? Number(f.value) : ''); const n = Number(count.value); stageInputs(Array.from({length:n}, (_, i) => previous[i] ?? '')); markDirty(); budgetHint(); });
  root.querySelectorAll<HTMLButtonElement>('[data-reversal-preset]').forEach(button => on(button, 'click', () => {
    ...
    count.value = '4'; stageInputs(preset.stageShares); budgetHint(); message('参考价格与份额已填入，可继续修改。资金上限和运行时间保持你的设置。');
  }));
