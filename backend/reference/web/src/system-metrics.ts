...
export function renderSystemMetrics(data:SystemMetrics|null,error:string|null){
  ...
  const signature=JSON.stringify({data,error,freshnessBucket:Math.floor(Date.now()/5000)});
  if(root.dataset.renderSignature===signature)return;
  root.dataset.renderSignature=signature;
  if(data?.asOf!==null&&data&&Date.now()/1000-data.asOf>5)error=error||'服务器状态已过期';
