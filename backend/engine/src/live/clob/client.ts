...
export interface SubmitOrderResult {
  ...
  ackLatencyMs?: number;
  /** Time spent generating the authenticated L2 headers for the POST. */
  l2HeaderLatencyMs?: number;
  /** Time from POST write until CLOB response headers arrive. */
  responseHeadersLatencyMs?: number;
  /** Time spent consuming the CLOB response body after headers arrive. */
  responseBodyLatencyMs?: number;
  /** HTTP POST round trip, including response body consumption. */
  postLatencyMs?: number;
  triggerToPostLatencyMs?: number;
  decisionToPostLatencyMs?: number;
  reactionLatencyMs?: number;
  /** Time spent resolving signing metadata before the order was built. */
  riskMetadataLatencyMs?: number;
  /** Last phase reached when a submission failed or became unknown. */
...
type RequestTimingSink = (timing: RequestTiming) => void;
...
export class ClobWrapper {
  ...
  private async fetchJson(
    ...
    onTiming?: (timing: Omit<RequestTiming, "requestStartMonoMs" | "l2HeaderLatencyMs">) => void,
  ): Promise<unknown> {
    const timeout = AbortSignal.timeout(timeoutMs);
    const request: RequestInit = { ...init,
      signal: init.signal ? AbortSignal.any([init.signal, timeout]) : timeout };
    const requestStartMonoMs = performance.now();
    const deadline = requestStartMonoMs + timeoutMs;
    onRequestStart?.(requestStartMonoMs);
    // AbortSignal.timeout does not guarantee that consuming an already-open
    // response body returns promptly. Race both phases against the same hard
    // deadline so an exchange-side delay cannot block the strategy loop.
    const remaining = () => Math.max(1, deadline - performance.now());
    const response = await withTimeout(
      fetch(`${DEFAULT_HOST}${path}`, request), remaining(), `CLOB ${init.method ?? "GET"} ${path}`,
      request.signal ?? undefined,
    );
    const responseHeadersAt = performance.now();
    const text = await withTimeout(
      response.text(), remaining(), `CLOB ${init.method ?? "GET"} ${path} response body`, request.signal ?? undefined,
    );
    const completedAt = performance.now();
    onTiming?.({ responseHeadersLatencyMs: Math.max(0, responseHeadersAt - requestStartMonoMs),
      responseBodyLatencyMs: Math.max(0, completedAt - responseHeadersAt),
      postLatencyMs: Math.max(0, completedAt - requestStartMonoMs) });
    let payload: unknown = {};
    if (text) {
      try {
  ...
  private async l2Json(
    ...
    return this.fetchJson(path, {
      ...
      body,
    }, this.requestTimeoutMs, onRequestStart, timing => onTiming?.({ requestStartMonoMs: 0, ...timing }));
  }
  ...
  private async postSignedOrder(
    ...
    return (await this.l2Json(POST_ORDER_PATH, "POST", payload, onRequestStart, onTiming)) as OrderResponse;
  }
          ...
          (async () => {
            ...
            for (const token of market.t) {
              if (token?.t) {
                ...
                const delaySeconds = Number(market.sd);
                const takerDelayMs = market.itode === true && Number.isFinite(delaySeconds) && delaySeconds >= 0
                  ? delaySeconds * 1000 : 0;
                this.feeRulesByToken.set(token.t, { rate, exponent, takerDelayMs });
              }
            }
            this.orderVersion = version;
            // The SDK treats a missing `nr` field as false and caches that
            // result while loading the market. Mirror that behavior locally.
            // Leaving the entry empty makes the first order issue a second
            // metadata request on the hot path; if that request waits for its
            // timeout, it is added directly to the order round trip.
            const negRisk = market.nr === true;
            for (const token of market.t) {
  ...
  async submitOrder(args: SubmitOrderArgs): Promise<SubmitOrderResult> {
    ...
    let failurePhase: SubmitOrderResult["failurePhase"] = "risk_metadata";
    let riskMetadataLatencyMs: number | undefined;
    let l2HeaderLatencyMs = 0;
    let responseHeadersLatencyMs = 0;
    let responseBodyLatencyMs = 0;
    let postLatencyMs = 0;
    let activePostStarted: number | undefined;
    const riskStarted = performance.now();
    try {
      const negRisk = await this.negRisk(args.tokenId);
      riskMetadataLatencyMs = performance.now() - riskStarted;
      let signLatencyMs = 0;
      let ackLatencyMs = 0;
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        let postStarted: number | undefined;
        let measuredPostLatency: number | undefined;
        failurePhase = "post";
        const orderType = args.timeInForce === "FOK" ? OrderType.FOK
          : args.timeInForce === "FAK" ? OrderType.FAK : OrderType.GTC;
        const resp = await this.postSignedOrder(order, orderType, args.postOnly ?? true, requestStartedAt => {
          ackStarted = requestStartedAt;
          postStarted = requestStartedAt;
          activePostStarted = requestStartedAt;
          triggerToPostLatencyMs ??= args.triggerReceivedAtMonoMs == null
            ? undefined : Math.max(0, requestStartedAt - args.triggerReceivedAtMonoMs);
          decisionToPostLatencyMs ??= args.decisionAtMonoMs == null
            ? undefined : Math.max(0, requestStartedAt - args.decisionAtMonoMs);
          postAttempted = true;
        }, timing => {
          measuredPostLatency = timing.postLatencyMs;
          if (timing.postLatencyMs != null) activePostStarted = undefined;
          if (timing.l2HeaderLatencyMs != null) l2HeaderLatencyMs += timing.l2HeaderLatencyMs;
          if (timing.responseHeadersLatencyMs != null) responseHeadersLatencyMs += timing.responseHeadersLatencyMs;
          if (timing.responseBodyLatencyMs != null) responseBodyLatencyMs += timing.responseBodyLatencyMs;
        });
        postAttempted = false;
        if (measuredPostLatency != null) postLatencyMs += measuredPostLatency;
        else if (postStarted != null) postLatencyMs += Math.max(0, performance.now() - postStarted);
        activePostStarted = undefined;
        ...
        if (success || attempt > 0 || !isVersionMismatch(resp)) {
          return {
            ...
            signLatencyMs,
            ackLatencyMs,
            l2HeaderLatencyMs: l2HeaderLatencyMs || undefined,
            responseHeadersLatencyMs: responseHeadersLatencyMs || undefined,
            responseBodyLatencyMs: responseBodyLatencyMs || undefined,
            postLatencyMs: postLatencyMs || undefined,
      ...
      if (riskMetadataLatencyMs == null && failurePhase === "risk_metadata") {
        riskMetadataLatencyMs = performance.now() - riskStarted;
      }
      if (activePostStarted != null) postLatencyMs += Math.max(0, performance.now() - activePostStarted);
      return {
        success: false,
