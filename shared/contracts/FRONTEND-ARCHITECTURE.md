...
页面接入顺序固定为：`preview-core` → `demo-data` → `view-model` → `preview-store` → `ws-client` → `api-adapter` → 页面脚本。生产挂载时只需把 `window.__POLY_PREVIEW_CONFIG__` 放在这些脚本之前，例如 `{ mode: "backend", apiBase: "", apiFlavor: "contract" }`。

## 数据流

1. REST 首次加载页面快照和慢数据。
2. 市场行情/盘口、运行状态、订单事件分别使用独立 WebSocket（或 SSE）频道。
