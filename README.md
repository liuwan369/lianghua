...
本项目用于 Polymarket BTC Up/Down 5 分钟市场的公开行情采集、证据回放、纸面模拟及受控交易执行。当前先交付与策略无关的交易平台，策略接入暂停。主节点位于 AWS 都柏林 `eu-west-1`，公网控制台为：
...
页面纸面交易启停已接线，实盘控制仍锁定。受控真实下单、成交、撤单和一笔 Deposit Wallet 零兑付赎回属于历史证据；真实部分成交、断线中成交、正兑付赎回、完整资金基线与奖励到账尚未验收。前端已连接账户和运行数据；五档深度、分段延迟和自动调整原因仍未完整展示。
