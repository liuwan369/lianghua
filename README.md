...
本项目用于 Polymarket BTC Up/Down 5 分钟市场的行情、订单、账户和策略执行。2026-09-17 主线切换为 BTC 五分钟反转策略，停止做市方向，复用公共交易底座。当前已实现反转插件、配置/控制API与新页面，已发布程序并进行线上接口核对，部署事实见 docs/CURRENT-STATUS.md。主节点位于 AWS 都柏林 `eu-west-1`，公网控制台为：
...
反转插件、自动换场、状态恢复、前端和外部资金流日内修正已实现；费用预算、未知提交身份、终态恢复及自动赎回代码已接线。剩余验收是用户发起的最低金额真实订单、实际充值提现及赎回到账；历史交易不等于新策略已验收。采用短时功能测试和小额真实接口验证，不新增回测、连续paper、多日观察、第三日期或午夜等待前置。
