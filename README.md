...
本项目用于 Polymarket BTC Up/Down 5 分钟市场的行情、订单、账户和策略执行。2026-09-17 主线切换为 BTC 五分钟反转策略，停止做市方向，复用公共交易底座。当前完成备份、技术评估与开发规划，新策略和页面尚未实现。主节点位于 AWS 都柏林 `eu-west-1`，公网控制台为：
...
- 正式控制台已有总览、自动交易、策略、收益、设置、任务视图，订单归入自动交易页；反转专用布局和10/20/50分页待实现。
- 现有配置接口及会话策略草稿可用；反转参数服务端持久化、方案互斥与下一场生效待实现，旧做市参数不作为新策略设置。
...
控制台启停、进程恢复和日志已接入独立平台 CLI；默认只观察行情，不加载策略。旧 `live.js run` 仅保留显式兼容入口。

待完成反转插件、自动换场、策略状态恢复、未知订单查回、真实费用、正兑付赎回到账及前端改造。历史真实下单、撤单和成交不等于新策略已验收。采用短时功能测试和小额真实接口验证，不新增回测、连续paper、多日观察、第三日期或午夜等待前置。

原50美元本金/30美元日损额度可用于接口验证，无法覆盖原版70/75四阶段。实现与资金配置分开推进，完整原版启动条件见新规划。当前没有新策略已上线或盈利的结论。
